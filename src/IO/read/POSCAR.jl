...
"""
	read(path::AbstractString/io::IO, ::Type{POSCAR}; period = Bool[1, 1, 1]) -> Cell

Read POSCAR.vasp, return a `Cell` object.
`period` represents the periodicity in three directions, should be a vector with 3 elements, and its elements should be `1` or `0`.
"""
